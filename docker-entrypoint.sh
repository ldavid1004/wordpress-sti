...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	...
	if ! [ -e index.php -a -e wp-includes/version.php ]; then
		...
		tar cf - --one-file-system -C /usr/src/wordpress . | tar --no-overwrite-dir -m -xf -
...
# make sure that apache is run as the same user executing this
# unnecessary since forced to have a user id by docker
# export APACHE_RUN_USER=$(id -n --user)
# export APACHE_RUN_GROUP=$(id -n --group)

echo "---> /var/lock/apache2 <---"
ls -al /var/lock/apache2

echo "--->staring to sleep for 10 sec so that this can be stopped<---"
sleep 20
echo "--->finished sleeping for 10 sec<---"

echo "--->staring to sleep for 5 sec so that this can be stopped<---"
sleep 5
echo "--->finished sleeping for 5 sec<---"
